...
Though I worked on Oxalis throughout my entire break, I also made sure to carve out sufficient time to touch grass and explore.
...
Packaging local inference into desktop applications is not a common pattern right now, but it does work, and you do need to be cautious in implementing it even with the help of LLMs. For Oxalis, I packaged llama.cpp directly into the application as a side-car. I chose it specifically because it would be easier to port the product to Windows as well.

### Local Inference Is Not Mature

llama.cpp has a pretty hard job of having to keep up with all of the new model releases that happen. For example, after the Gemma 4 release, Gemma 4 12B was also released, and MTP drafters for all models in the family were also released. They are good at keeping up, but bugs were more frequent than I expected.
...
After creating the dataset, I worked with Fable to set up an MLX training pipeline on my Mac Studio with unsloth, and even trained new MTP drafters with comparable acceptance rates to the stock Gemma 4 e2b/e4b models.

I'm still iterating on this model (Shamrock) but for now it benchmarks way better than stock Gemma 4 E2B/E4B for Oxalis workloads, both in accuracy and speed. For transcribing YouTube videos to vocab breakdowns specifically, I got Shamrock E2B to eval comparably to Gemma 4 E4B, which was great especially for edge inference where the snappiness of E2B is much preferred.
...
Looking at Oxalis at the surface level, I think it's an app some people would claim can be vibe-coded in one sitting. While I think you actually could get some prototype that resembles it, I'm confident you can't achieve the same quality in one sitting.
...
<figure>
  ...
  <figcaption>Commits are never a good measure of productivity, but if you're mostly prompting specs it can be an (unreliable) proxy of decisions made.</figcaption>
</figure>
