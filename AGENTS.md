...
| **Charts**                   | A ` ```chart ` fenced block whose body is a JSON spec renders as a theme-aware inline SVG via `SignalChart` (`src/screens/Signals/SignalChart.tsx`). Types: `dumbbell` (before/after per row: `from`/`to` legend labels, `domain`, `ticks`, `rows[{label, group?, from, to}]`) and `line` (`x{domain, ticks, labels?}`, `y{domain, ticks, precision?}`, `series[{label, tone?, dash?, markers?, annotate?, points}]`). Optional `caption` and `note`. Styled under `.signal-prose .signal-chart*` in `index.css`; scrolls sideways below 560px. Stripped from list excerpts by `preview.ts` and replaced with a captioned placeholder in the RSS feed. |
| **Tables**                   | GFM tables are styled under `.signal-prose table` and wrapped in `.signal-table-scroll`, which scrolls sideways on narrow viewports instead of widening the page. Use `---:` column alignment for numeric columns — right-aligned cells sit flush to the table edge with hairline rules.                                                                                                                                                                                                                                                                                                                                                               |
