...
| **Image captions**           | Wrap the `<img>` in `<figure>` and add `<figcaption>` for caption text. Styled under `.signal-prose figcaption`; the wrapper and caption are stripped from the collapsed list excerpt by `preview.ts`.                                         |
